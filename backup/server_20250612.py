...
async def check_task(request: CheckTaskRequest):
    ...
    ### Check task ###
    ##################
    if len(check_result_scenarios) == 0:
        raise HTTPException(status_code=500, detail="No scenario check result")
    elif len(check_result_scenarios) == 1:
        # # 如果只有一个场景，直接返回结果到“检查结论”字段
        # return CheckTaskResponse(
        #     检查结论=check_result_scenarios[0],
        # )
        
        # 如果只有一个场景，解析场景检查结果并返回
        check_result_list = check_result_scenarios[0].split("\n")
        return CheckTaskResponse(
            检查结论=check_result_list[2].strip("- 检查结论：").strip(),
            风险等级=check_result_list[3].strip("- 风险等级：").strip(),
            风险依据=check_result_list[4].strip("- 风险依据：").strip(),
        )
    else: 
        output_json_format = (
            "```json\n"
